...
Two deployment options are available for this platform. The first is a production ready solution for periodic updates which uses Docker-Compose along with Celery, Redis and Nginx. The second option is a simpler one which uses Django's webserver excluding Celery or Docker. The second deployment option is perfect for researchers who just want to retrive batches of data for a study few times manually.
